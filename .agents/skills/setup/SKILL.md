...
This skill assumes `@cyanheads/mcp-ts-core init` has already run. The CLI created the project's `AGENTS.md` and `AGENTS.md` (identical content), copied external skills to `skills/`, and scaffolded the directory structure with echo definitions as starting points. This skill covers what was created and what to do next.

## Agent Protocol File

The init CLI generates both `AGENTS.md` and `AGENTS.md` with identical content. Keep the one your agent uses, discard the other:

- **Codex** — keep `AGENTS.md`, discard `AGENTS.md`
- **All other agents** (Codex, Cursor, Windsurf, etc.) — keep `AGENTS.md`, discard `AGENTS.md`
...
For the full framework API, read:
    ...
    node_modules/@cyanheads/mcp-ts-core/AGENTS.md
...
```text
AGENTS.md                                       # Agent protocol (project-specific)
...
3. **Verify agent protocol placeholders** — if the `init` CLI was run without a `[name]` argument, `{{PACKAGE_NAME}}` may remain as a literal in `AGENTS.md`/`AGENTS.md` and `package.json`. Replace it with the actual server name.

## Checklist

- [ ] Agent protocol file selected — keep `AGENTS.md` or `AGENTS.md`, discard the other
- [ ] `{{PACKAGE_NAME}}` placeholders replaced in agent protocol file (if not auto-substituted by init)
- [ ] Core framework AGENTS.md read (`node_modules/@cyanheads/mcp-ts-core/AGENTS.md`)
- [ ] Unused echo definitions cleaned up (and unregistered from `src/index.ts`)
- [ ] Skills copied to agent directory (`cp -R skills/* .Codex/skills/` or equivalent)
