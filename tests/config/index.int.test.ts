...
describe('Configuration Service', () => {
  ...
  it('should include NCBI configuration with defaults and overrides', async () => {
    process.env.NCBI_API_KEY = 'test-ncbi-key';
    process.env.NCBI_ADMIN_EMAIL = 'admin@example.com';
    process.env.NCBI_REQUEST_DELAY_MS = '500';
    process.env.NCBI_MAX_RETRIES = '5';
    process.env.NCBI_TIMEOUT_MS = '45000';

    const { parseConfig } = await import('../../src/config/index.js');
    const config = parseConfig();

    expect(config.ncbiApiKey).toBe('test-ncbi-key');
    expect(config.ncbiAdminEmail).toBe('admin@example.com');
    expect(config.ncbiRequestDelayMs).toBe(500);
    expect(config.ncbiMaxRetries).toBe(5);
    expect(config.ncbiTimeoutMs).toBe(45000);
    expect(config.ncbiToolIdentifier).toBeTruthy();
  });
});
